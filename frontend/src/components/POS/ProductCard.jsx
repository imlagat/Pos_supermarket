...
export default function ProductCard({ product, onAdd }) {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 hover:shadow-lg transition-all cursor-pointer bg-white">
      ...
      <div className="text-lg font-bold text-orange-500 mt-1">Ksh {price}</div>
      <div className="flex gap-2 mt-2">
        <button
          onClick={handlePieceClick}
          className={`text-xs px-2 py-1 rounded ${unitType === 'piece' ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-700'}`}
        >
        ...
        {units.map(unit => (
          <button
            ...
            className={`text-xs px-2 py-1 rounded ${unitType === 'crate' && selectedUnit?.id === unit.id ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
      ...
      <button
        onClick={handleAdd}
        className="mt-3 w-full bg-orange-50 text-orange-500 py-1.5 rounded-lg text-sm font-medium hover:bg-orange-100 transition flex items-center justify-center gap-1"
      >
        <Plus size={14} /> Add
      </button>
    </div>
